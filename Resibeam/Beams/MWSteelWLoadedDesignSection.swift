//
//  MWSteelWLoadedDesignSection.swift
//  ResiBeam_V0.0.7
//
...
class MWSteelWLoadedDesignSection: NSObject {
    ...
    func setShear(_ theShearVal:Double, sectionData:MWSteelWSectionDesignData){
        designShear = theShearVal
        updateShearStress(sectionData)
    }
    
    func updateShearStress(_ sectionData:MWSteelWSectionDesignData){
        ...
        //fv = 3 * V / (2 * b * d) for rectangular section
           ...
           // Swift.print("shear stress was calculated using rectangle")
    ...
    func setMoment(_ theMomentVal:Double, sectionData:MWSteelWSectionDesignData){
        designMoment = theMomentVal
        updateBendingStress(sectionData)
    }
    
    func updateBendingStress(_ sectionData:MWSteelWSectionDesignData){
        //fb = M/S this works for steel and wood
        bendingStress = designMoment * 12 / sectionData.sectionModulus
    }
    
    func setDeflection(_ theDeflectionVal:Double, IOriginal:Double, EOriginal:Double, EAdjust:Double, sectionData:MWSteelWSectionDesignData){
