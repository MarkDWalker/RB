...
class vc_LVLBeamDesignPanel: NSViewController, NSTableViewDataSource, NSTableViewDelegate, MWFactorReceiver {
    ...
    override func viewWillAppear() {
        ...
        delegate?.updateSaveDocWithSelectedDesignTabIndex(selectedTabIndex: 1)
        updateStatus()
    }
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        if segue.identifier == "BeamCalculations"{
            ...
            vc_Calcs.lvlBeamDesign = self.design
            vc_Calcs.calcType = "LVL"
        }
    ...
    @IBAction func onFbFactorTableEdit(_ sender: AnyObject) {
        ...
        guard let doubleVal = Double(sender.stringValue)else{
            updateTables()//clears out the bad data
            return
        }
    ...
    @IBAction func onFvFactorTableEdit(_ sender: AnyObject) {
        ...
        guard let doubleVal = Double(sender.stringValue)else{
            updateTables()//clears out the bad entry
            return
        }
    ...
    func updateStatus(){
        ...
        if design.LVLDesignSectionCollection.count == 0{
        ...
        for i in 0...design.LVLDesignSectionCollection.count-1{
